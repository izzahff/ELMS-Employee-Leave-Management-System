...
<title>Admin Forgot Password - IMNSB Employee Leave Management System</title>
...
<body>
	<div class="container">
		<div class="auth-card">
			...
			<div class="auth-form">
				<h2 id="form-title">Admin Forgot Password</h2>
				
				<div class="auth-links">
					<a href="/ELMS_3.0/Admin/AdminLogin.jsp" class="auth-link">← Back to Admin Login</a>
				</div>
				...
				<div id="email-form" class="step-form">
					...
					<div class="form-description">
						<p>Enter your admin email address and we'll send you an OTP to reset your password.</p>
					</div>
					
					<form action="/ELMS_3.0/AdminForgotPasswordController" method="post" id="emailForm">
				...
				<div id="otp-form" class="step-form" style="display: none;">
					...
					<div class="form-description">
						<p>Enter the 6-digit OTP sent to your admin email address.</p>
						<p class="email-sent">OTP sent to: <span id="sent-email"></span></p>
					</div>
					
					<form action="/ELMS_3.0/AdminForgotPasswordController" method="post" id="otpForm">
				...
				<div id="password-form" class="step-form" style="display: none;">
					...
					<form action="/ELMS_3.0/AdminForgotPasswordController" method="post" id="passwordForm">
	...
	<!-- Add CSS for forgot password specific styling -->
	<style>
	 ...
	 body {
        ...
        /* Password input container - CRITICAL for positioning */
	...
	function startOTPTimer(duration) {
	    console.log('Starting admin OTP timer with duration:', duration);
	    ...
	    timerInterval = setInterval(() => {
	        ...
	        if (time <= 0) {
	            ...
	            // Show expiry message
	            showTimerMessage('Admin OTP has expired. Please request a new one.', 'error');
	        }
	    }, 1000);
	    
	    console.log('Admin timer started successfully');
	}
	...
	function updateTimerDisplay(time, timer) {
	    ...
	    console.log('Admin timer updated:', displayTime, 'Element visible:', timer.offsetParent !== null);
	...
	function showStep(stepNumber) {
	    console.log('Showing admin step:', stepNumber);
	    ...
	    if (targetForm) {
	        ...
	        console.log('Admin form displayed:', forms[stepNumber - 1]);
	    } else {
	        console.error('Target form not found:', forms[stepNumber - 1]);
	    }
	    
	    // Update form title
	    const titles = ['Admin Forgot Password', 'Verify Admin OTP', 'Create New Admin Password'];
	    ...
	    if (stepNumber === 2) {
	        console.log('Admin step 2 - preparing to start timer');
	        
	        // Wait for DOM to be fully rendered
	        setTimeout(() => {
	            const timer = document.getElementById('timer');
	            if (timer) {
	                console.log('Admin timer element found, starting timer');
	                ...
	                console.error('Admin timer element not found after DOM render');
	            }
	        }, 200);
	    }
	}
	...
	document.addEventListener('DOMContentLoaded', function() {
	    ...
	    if (step === '2' && email) {
	        console.log('Loading admin OTP step with email:', email);
	        showStep(2);
	        ...
	        setTimeout(() => {
	            if (message === 'otp_sent') {
	                showTimerMessage('Admin OTP has been sent to your email address', 'success');
	            } else if (message === 'otp_resent') {
	                showTimerMessage('New admin OTP has been sent to your email address', 'success');
	            }
	        }, 500);
	    ...
	    if (window.location.search.includes('step=2')) {
	        ...
	        setTimeout(() => {
	            ...
	            if (timer && timer.textContent === '05:00') {
	                console.log('Admin timer not started automatically, starting manually');
	                startOTPTimer(300);
	            }
	        }, 1000);
	    }
	    ...
	    if (resendButton) {
	        resendButton.addEventListener('click', function() {
	            ...
	            if (email) {
	                ...
	                window.location.href = '/ELMS_3.0/AdminForgotPasswordController?action=resendOTP&email=' + encodeURIComponent(email);
	            }
	        });
	    }
	...
	function debugAdminTimer() {
	    console.log('=== ADMIN TIMER DEBUG ===');
	    const timer = document.getElementById('timer');
	    ...
	    // Test timer start
	    if (timer) {
	        console.log('Testing admin timer start...');
	        startOTPTimer(10); // 10 second test
	    }
	}
	
	// Force timer to show
	function forceShowAdminTimer() {
	    const timer = document.getElementById('timer');
	    if (timer) {
	        timer.style.display = 'inline';
	        timer.style.visibility = 'visible';
	        ...
	        console.log('Admin timer forced to show');
	    }
	}
	
	// Make functions available globally
	window.debugAdminTimer = debugAdminTimer;
	window.forceShowAdminTimer = forceShowAdminTimer;
