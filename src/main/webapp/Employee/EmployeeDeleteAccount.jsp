...
<%@ page import="elms.DAO.EmployeeDAO" %>
...
<%
    ...
    // Get newly updated applications count for notification bell
    ...
    if ("POST".equals(request.getMethod())) {
        ...
        if (password != null && confirmDelete != null) {
            ...
            Employee validatedEmployee = EmployeeDAO.validateLogin(employee.getEmployeeId(), password);
            
            if (validatedEmployee != null) {
                // Password is correct, proceed with account deletion
                try {
                    boolean deleted = deleteEmployeeAccount(employee.getEmployeeId());
                    if (deleted) {
                        // Clear session and redirect to login with success message
                        session.invalidate();
                        response.sendRedirect("/ELMS_3.0/Employee/EmployeeLogin.jsp?message=Account deleted successfully");
                        return;
                    } else {
                        errorMessage = "Failed to delete account. Please try again.";
                    }
                } catch (Exception e) {
                    System.err.println("Error deleting employee account: " + e.getMessage());
...
<%!
    // Helper method to delete employee account
    private boolean deleteEmployeeAccount(String employeeId) {
        ...
        try {
            ...
            // Delete related records first (leave applications, etc.)
            String deleteLeaveApps = "DELETE FROM leaveapplication WHERE employeeid = ?";
            pstmt = conn.prepareStatement(deleteLeaveApps);
            ...
            pstmt = conn.prepareStatement(deleteEmployee);
            pstmt.setString(1, employeeId);
            int rowsDeleted = pstmt.executeUpdate();
            
            if (rowsDeleted > 0) {
                conn.commit();
                System.out.println("Employee account deleted successfully: " + employeeId);
                return true;
            } else {
                conn.rollback();
                return false;
                    ...
                    <div class="settings-form-card danger">
                        <div class="delete-warning">
                            ...
                            <ul>
                                ...
                                <li>Complete leave history and applications</li>
                                <li>All associated records in the system</li>
                                <li>Access to your employee account</li>
                            </ul>
                        ...
                        <form id="deleteAccountForm" method="POST" action="">
                            ...
                            <div class="form-buttons">
                                <a href="/ELMS_3.0/Employee/EmployeeSettings.jsp" class="btn btn-secondary">Cancel
                                </a>
...
<div id="confirmModal" class="popup-overlay" style="display: none;">
        ...
        <div class="popup-body">
            <div style="text-align: center; padding: 1rem 0;">
                ...
                <p style="color: #495057; font-size: 1rem; margin: 0 0 1rem 0; line-height: 1.5;">
                    Are you absolutely sure you want to delete your employee account?
                </p>
                ...
                <div class="consequences-list">
                    ...
                    <ul>
                        ...
                        <li>Remove your complete leave history and applications</li>
                        <li>Delete all associated records in the system</li>
                        <li>Revoke access to your employee account</li>
    ...
    <script>
        ...
        // Notification function for leave history navigation
