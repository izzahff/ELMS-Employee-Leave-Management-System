...
/**
 * Servlet to generate PDF view of leave applications
 */
@WebServlet("/leave-application-pdf")
public class LeaveApplicationPDFServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("=== LEAVE APPLICATION PDF REQUEST ===");
        ...
        try {
            ...
            if (!employee.getEmployeeId().equals(application.getEmployeeid())) {
                response.sendError(HttpServletResponse.SC_FORBIDDEN, "Access denied");
                return;
            }
            
            // Get employee details (in case we need more info than what's in session)
            Employee applicant = EmployeeDAO.getEmployeeByIdOnly(application.getEmployeeid());
            if (applicant == null) {
                applicant = employee; // Fallback to session employee
            }
            ...
            generatePDFView(response, application, applicant, leaveType, reviewer);
            
        } catch (Exception e) {
            System.err.println("❌ Error generating PDF view: " + e.getMessage());
            e.printStackTrace();
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Error generating PDF view");
        }
    }
    
    private void generatePDFView(HttpServletResponse response, LeaveApplication application, 
                                Employee applicant, LeaveType leaveType, Manager reviewer) throws IOException {
        ...
        System.out.println("🔍 GENERATING EMPLOYEE PDF - DEBUG INFO:");
        ...
        out.println("    <title>Leave Application Form - " + application.getApplicationid() + "</title>");
        
        // Use same CSS as Admin version (without admin-specific styles)
        out.println("    <style>");
        out.println("        @page { size: A4; margin: 10mm; }");
        out.println("        @media print { .no-print { display: none !important; } body { print-color-adjust: exact; } }");
        out.println("        body { font-family: 'Arial', 'Times New Roman', serif; line-height: 1.2; color: #000; max-width: 210mm; margin: 0 auto; padding: 0; background: white; font-size: 12px; }");
        ...
        out.println("        .logo-section { width: 60px; height: 60px; border: none; margin-right: 15px; display: flex; align-items: center; justify-content: center; background: white; font-size: 8px; text-align: center; border-radius: 3px; overflow: hidden; }");
        out.println("        .logo-section img { max-width: 100%; max-height: 100%; object-fit: contain; border-radius: 2px; }");
        ...
        out.println("        .print-button:hover { background: #0056b3; } .print-button.secondary { background: #6c757d; } .print-button.secondary:hover { background: #545b62; }");
        ...
        out.println("        <h3>🖨️ Professional Leave Application Form</h3>");
        out.println("        <p>This form is optimized for printing. Click 'Print Form' to get a printout.</p>");
        ...
        out.println("            <div>IMNSB Employee Leave Management System | Form Generated: " + new SimpleDateFormat("dd/MM/yyyy HH:mm").format(new Date()) + " | Application ID: " + application.getApplicationid() + "</div>");
        out.println("        </div>");
        ...
        out.flush();
        System.out.println("✅ PDF view generated successfully for application: " + application.getApplicationid());
    ...
    private String formatDateTime(String dateTimeStr) {
        ...
        try {
            SimpleDateFormat inputFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            SimpleDateFormat outputFormat = new SimpleDateFormat("MMMM dd, yyyy 'at' hh:mm:ss a");
